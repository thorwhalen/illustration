...
#
# One copy of each provider's canned response, shared with the TypeScript twin:
# ``schema/fixtures/<provider>.payload.json`` is the hand-authored SSOT, and
# ``illustration export-schema`` derives ``<provider>.expected.json`` from it
# (what *this* package normalises the payload into), which ``ts/`` replays in
# its parity tests. A payload edited here therefore changes both suites.

CANNED_PAYLOADS_DIR = os.path.join(os.path.dirname(__file__), "schema", "fixtures")


def canned_payload(provider: str) -> dict:
    """Load ``schema/fixtures/<provider>.payload.json`` (a decoded provider page)."""
    import json

    with open(os.path.join(CANNED_PAYLOADS_DIR, f"{provider}.payload.json")) as f:
        return json.load(f)


@pytest.fixture
def openverse_payload():
    """A canned 2-result Openverse page (page 1 of 2) (from ``schema/fixtures/openverse.payload.json``)."""
    return canned_payload("openverse")


@pytest.fixture
def pexels_payload():
    """A canned 1-result Pexels response (from ``schema/fixtures/pexels.payload.json``)."""
    return canned_payload("pexels")


@pytest.fixture
def pixabay_payload():
    """A canned 1-result Pixabay response (from ``schema/fixtures/pixabay.payload.json``)."""
    return canned_payload("pixabay")


@pytest.fixture
def wikimedia_payload():
    """A canned MediaWiki response: one image + one non-image (audio) to drop (from ``schema/fixtures/wikimedia.payload.json``)."""
    return canned_payload("wikimedia")
