...
M3’s [`illustration.curate()`](illustration.md#illustration.curate) picks the best image for *one* beat in isolation.
...
[`illustration.reranking.embed_images()`](illustration.reranking.md#illustration.reranking.embed_images), behind the `[rerank]` extra);
...
via [`illustration.search()`](illustration.md#illustration.search), so pools carry relevance `.score`). For
